...
def fetch_news():
    news_url = "https://newsapi.org/v2/top-headlines?country=us&apiKey=85f51cda38534e3aa92cf127d7857135"  # Replace with your NewsAPI key
    response = requests.get(news_url).json()
    

    if response.get("status") != "ok":
        speak(f"Sorry, I couldn't fetch the news headlines.")
        print(f"Error fetching news: {response.get('message', 'Unknown error')}")
        return None  # Return None if there was an issue fetching news

    try:
        news_list = response.get('articles', [])
        if not news_list:
            speak("Sorry, no news articles found.")
            return None  # Return None if no articles were found

        # Limit to first three articles
        news_list = news_list[:3]

        for idx, article in enumerate(news_list, 1):
            speak(f"Number {idx}: {article['title']}")
            print(f"Number {idx}: {article['title']}")

        return news_list  # Return the list of news articles
    except Exception as e:
        speak(f"Sorry, there was an error fetching the news headlines.")
...
def fetch_weather():
    ...
    api_key = "736a14fc7a8dfda4cb93a4a6201110f7"  # Replace with your OpenWeatherMap API key
